	...
	if (applied_pos >= 0) {
		...
		if ((leading != frag->leading ||
			...
			fprintf_ln(stderr, _("Context reduced to (%lu/%lu)"
				   ...
				   xulong_t(leading), xulong_t(trailing), applied_pos+1);
...
static int apply_fragments(struct apply_state *state, struct image *img, struct patch *patch)
{
	...
	while (frag) {
		...
		if (apply_one_fragment(state, img, frag, inaccurate_eof, ws_rule, nth)) {
			error(_("patch failed: %s:%lu"), name, xulong_t(frag->oldpos));
			if (!state->apply_with_reject)
				return -1;
			frag->rejected = 1;
		}
 ...
 */
static int try_create_file(const char *path, unsigned int mode, const char *buf, size_t size)
{
	int fd, res;
