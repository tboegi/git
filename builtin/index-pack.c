			       ...
			       enum object_type type, unsigned char *sha1)
{
	...
	if (!is_delta_type(type)) {
	  	hdrlen = xsnprintf(hdr, sizeof(hdr), "%s %lu", typename(type), xulong_t(size)) + 1;
		git_SHA1_Init(&c);
...
static void show_pack_info(int stat_only)
{
	...
	for (i = 0; i < nr_objects; i++) {
		...
		printf("%s %-6s %lu %lu %"PRIuMAX,
		       ...
		       typename(obj->real_type), xulong_t(obj->size),
		       xulong_t((obj[1].idx.offset - obj->idx.offset)),
